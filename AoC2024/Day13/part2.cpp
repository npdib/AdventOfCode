	...
	std::string run()
	{
		...
		while (InputTaker::get().moreToRead())
		{
			...
			else if (line.find('X') != std::string::npos)
			{
				...
				currentMachine.U = 10000000000000 + std::stoll(line.substr(firstEquals + 1, comma - firstEquals - 1));
				currentMachine.V = 10000000000000 + std::stoll(line.substr(secondEquals + 1));
